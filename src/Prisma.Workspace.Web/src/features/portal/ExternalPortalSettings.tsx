...
export function ExternalPortalSettings({project}:{project:ProjectSummary}){
  ...
  return <Section><header><Globe2 size={16}/><h2>Portal Externo e formulários</h2><small>{portalForm.isEnabled?'Publicado':'Desativado'}</small></header>
    <Form onSubmit={(event:FormEvent)=>{event.preventDefault();if(portalForm.boardId&&portalForm.accessModes)savePortal.mutate();}}>
      ...
      <label><span><input type="checkbox" checked={portalForm.isEnabled} onChange={event=>setPortalForm({...portalForm,isEnabled:event.target.checked})}/> Portal habilitado</span></label>
      <label><span><input type="checkbox" checked={portalForm.requiresAuthentication} onChange={event=>setPortalForm({...portalForm,requiresAuthentication:event.target.checked})}/> Exigir autenticação</span></label>

...
const Manual = styled.form`display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:12px 16px;border-bottom:1px solid ${({theme})=>theme.color.border};input{min-height:32px;padding:0 8px;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.md};font-size:13px}.wide{grid-column:1/-1}.submit{justify-self:end}`;
...
export function TaskTimePanel({workItemId,realizedHours}:{workItemId:string;realizedHours:number}){
  ...
  const refresh=async()=>{window.dispatchEvent(new Event('timer-change'));await Promise.all([
    client.invalidateQueries({queryKey:['time-entries',workItemId]}),
    client.invalidateQueries({queryKey:['active-timer']}),
    client.invalidateQueries({queryKey:['work-item',workItemId]})
  ]);};
  ...
  return <div>
    <Actions><Button type="button" $running={currentHere} disabled={toggle.isPending||anotherRunning} onClick={()=>toggle.mutate()}>{currentHere?<><Square size={11}/>Parar cronômetro</>:<><Play size={11}/>Iniciar cronômetro</>}</Button><Summary><Clock3 size={11}/> {realizedHours.toFixed(2)} h realizadas</Summary>{anotherRunning&&<Summary>Há um cronômetro ativo em outra tarefa.</Summary>}</Actions>
    ...
    {(toggle.error||add.error||entries.error)&&<Error>{((toggle.error||add.error||entries.error) as Error).message}</Error>}

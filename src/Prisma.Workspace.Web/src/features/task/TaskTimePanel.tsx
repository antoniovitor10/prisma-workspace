...
export function TaskTimePanel({workItemId,realizedHours}:{workItemId:string;realizedHours:number}){
  ...
  return <div>
    <Actions><Button type="button" $running={currentHere} disabled={toggle.isPending||running.isLoading||running.isError} onClick={()=>toggle.mutate()}>{currentHere?<><Square size={11}/>Parar cronômetro</>:<><Play size={11}/>Iniciar cronômetro</>}</Button><Summary><Clock3 size={11}/> {realizedHours.toFixed(2)} h realizadas</Summary>{anotherRunning&&<Summary>Ao iniciar aqui, o cronômetro da outra tarefa será encerrado automaticamente.</Summary>}</Actions>

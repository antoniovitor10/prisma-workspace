  ...
  return <Page><Grid>
    <Section><header><FolderCog size={16}/><h2>Dados do projeto</h2>{savedAt&&<small>Salvo às {savedAt}</small>}</header>
      <Form onSubmit={submit}>
        ...
        <label>Responsável<select required value={form.ownerId} onChange={e=>setForm({...form,ownerId:e.target.value})}><option value="">Selecione...</option>{membersQuery.data?.filter(item=>item.isActive).map(item=><option key={item.userId} value={item.userId}>{item.name}</option>)}</select></label>
        <label>Status{project.isArchived
          ?<select value={0} disabled title="Reative o projeto em Ciclo de vida para alterar o status"><option value={0}>Arquivado</option></select>
          :<select value={form.status} onChange={e=>setForm({...form,status:Number(e.target.value)})}>{statusOptions.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select>}</label>
    ...
    <Wide><header><Users size={16}/><h2>Membros e papéis</h2><small>{project.members?.length??0} pessoa(s)</small></header><List>{project.members?.map(item=><Row key={item.userId}><div><strong>{memberNames.get(item.userId)??item.userId}</strong><small>{item.userId===project.ownerId?'Responsável principal':'Membro do projeto'}</small></div><div style={{display:'flex',gap:7}}><select value={item.role} onChange={e=>saveMember.mutate({userId:item.userId,role:Number(e.target.value)})}>{projectRoles.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select>{item.userId!==project.ownerId&&<Button $danger aria-label="Remover membro" onClick={()=>removeMember.mutate(item.userId)}><Trash2 size={12}/></Button>}</div></Row>)}{!project.members?.length&&<Empty>Nenhum membro adicional.</Empty>}</List><InlineForm onSubmit={submitMember}><select required value={member.userId} onChange={e=>setMember({...member,userId:e.target.value})}><option value="">Adicionar membro...</option>{membersQuery.data?.filter(item=>item.isActive&&!project.members?.some(current=>current.userId===item.userId)).map(item=><option key={item.userId} value={item.userId}>{item.name}</option>)}</select><select value={member.role} onChange={e=>setMember({...member,role:Number(e.target.value)})}>{projectRoles.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select><Button><UserPlus size={13}/>Adicionar</Button></InlineForm></Wide>

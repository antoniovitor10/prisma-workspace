...
/* Menu de categorias a esquerda e o conteudo da categoria escolhida a direita. Em tela
   estreita o menu vira uma faixa horizontal rolavel acima do conteudo: como coluna fixa ele
   comeria a largura que os formularios precisam. */
const Layout = styled.div`
  display:grid; grid-template-columns:216px minmax(0,1fr); gap:18px; align-items:start;
  /* Quebra antes de 900px: com a coluna do menu ocupando 216px, o grid interno de duas
     colunas (minimo de 300px na segunda) estouraria a largura entre 900 e ~1050px. */
...
const Menu = styled.nav`
  ...
  @media(max-width:1080px){
    ...
    scrollbar-width:thin; >button{flex:0 0 auto;}
  }
`;
const MenuItem = styled.button<{ $active: boolean }>`
  display:flex; align-items:center; gap:9px; min-height:38px; padding:0 11px; border:0;
  border-radius:${({theme})=>theme.radius.md}; background:${({theme,$active})=>$active?`color-mix(in srgb, ${theme.color.brand} 10%, transparent)`:'transparent'};
  color:${({theme,$active})=>$active?theme.color.brand:theme.color.textMuted};
  font-size:13.5px; font-weight:750; text-align:left; white-space:nowrap; cursor:pointer;
...
const categorias: readonly [string, string, LucideIcon][] = [
  ['geral', 'Geral', FolderCog],
  ['pessoas', 'Pessoas e equipes', Users],
  ['classificacao', 'Classificação', Tags],
  ['fluxo', 'Fluxo de trabalho', Route],
  ['portal', 'Portal externo', Globe],
  ['historico', 'Histórico', Clock3],
];
const chavesValidas = new Set(categorias.map(([chave])=>chave));
...
export function ProjectSettings() {
  ...
  // A categoria vive na URL para que o botao voltar funcione e o link possa ser enviado
  // apontando direto para a secao, em vez de pedir "procura em configuracoes".
  const [searchParams, setSearchParams] = useSearchParams();
  const pedida = searchParams.get('secao') ?? '';
  const secao = chavesValidas.has(pedida) ? pedida : 'geral';
  const irPara = (chave: string) => {
    const proximos = new URLSearchParams(searchParams);
    proximos.set('secao', chave);
    ...
    <Menu aria-label="Categorias de configuração">
      {categorias.map(([chave, rotulo, Icone]) => (
        <MenuItem
          ...
          type="button"
          $active={secao === chave}
          aria-current={secao === chave ? 'true' : undefined}

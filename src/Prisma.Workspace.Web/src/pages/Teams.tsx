...
const Page = styled.div`max-width: 1180px; margin: auto; padding: 28px 32px 60px;`;
...
const Create = styled.form`
  display:flex; gap:8px; input{width:230px;min-height:38px;padding:0 10px;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.md};}
`;
const Button = styled.button`
  ...
  &:disabled{opacity:.55;}
`;
const Grid = styled.div`display:grid;gap:15px;`;
const Card = styled.section<{ $inactive:boolean }>`
  overflow:hidden;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.lg};background:${({theme})=>theme.color.surface};opacity:${({$inactive})=>$inactive?.72:1};
`;
const CardHeader = styled.header`
  display:flex;align-items:center;gap:12px;min-height:62px;padding:10px 17px;border-bottom:1px solid ${({theme})=>theme.color.border};background:${({theme})=>theme.color.neutral[50]};
  h2{font-size:15px;} p{margin-top:3px;color:${({theme})=>theme.color.textMuted};font-size:13px;} >span{margin-left:auto;display:flex;gap:5px;}
...
const Editor = styled.form`
  ...
  label{display:grid;gap:5px;color:${({theme})=>theme.color.textMuted};font-size:12px;font-weight:800;} input,select{min-height:36px;padding:0 9px;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.md};font-size:13.5px;}
  @media(max-width:760px){grid-template-columns:1fr 1fr;}
`;
const MembersHead = styled.div`display:grid;grid-template-columns:minmax(180px,1fr) 220px 120px 34px;gap:12px;padding:10px 17px;color:${({theme})=>theme.color.textMuted};font-size:12px;font-weight:800;text-transform:uppercase;@media(max-width:700px){display:none;}`;
const MemberRow = styled.div`
  display:grid;grid-template-columns:minmax(180px,1fr) 220px 120px 34px;gap:12px;align-items:center;min-height:52px;padding:8px 17px;border-top:1px solid ${({theme})=>theme.color.neutral[100]};font-size:13.5px;
  strong{display:flex;align-items:center;gap:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;} input{width:74px;min-height:32px;padding:0 7px;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.sm};}
  button{display:grid;width:30px;height:30px;place-items:center;color:${({theme})=>theme.color.textMuted};}
  @media(max-width:700px){grid-template-columns:1fr auto;.usage{display:none;}}
`;
...
const AddMember = styled.div`
  display:flex;gap:8px;padding:13px 17px;border-top:1px solid ${({theme})=>theme.color.border};select{min-width:240px;min-height:34px;padding:0 8px;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.md};font-size:13.5px;}
`;
const Projects = styled.div`
  display:flex;align-items:center;flex-wrap:wrap;gap:8px;padding:12px 17px;border-top:1px solid ${({theme})=>theme.color.border};color:${({theme})=>theme.color.textMuted};font-size:13px;
  >span{display:flex;align-items:center;gap:5px;font-weight:800;} label{display:flex;align-items:center;gap:5px;padding:5px 8px;border:1px solid ${({theme})=>theme.color.border};border-radius:${({theme})=>theme.radius.pill};background:${({theme})=>theme.color.surface};}
`;
...
function TeamCard({ team, users, projects, run }: {
  ...
  return <Card $inactive={!team.isActive}>
    ...
    {editing && <Editor onSubmit={submit}>
      ...
      <label>Líder<select value={form.leaderId} onChange={event=>setForm({...form,leaderId:event.target.value})}><option value="">Sem líder</option>{team.members.map(member=><option key={member.userId} value={member.userId}>{member.name}</option>)}</select></label>
    ...
    {team.isActive && <AddMember><select value={pick} onChange={event=>setPick(event.target.value)}><option value="">Adicionar membro...</option>{users.filter(user=>!team.members.some(member=>member.userId===user.id)).map(user=><option key={user.id} value={user.id}>{user.email??user.userName??user.id}</option>)}</select><Button disabled={!pick} onClick={()=>{if(pick){run(()=>api.addTeamMember(team.id,pick,team.defaultWeeklyCapacityHours));setPick('');}}}><Plus size={14}/>Adicionar</Button></AddMember>}

...
`;
const Footer = styled.footer`display:flex;min-height:34px;align-items:center;justify-content:flex-end;padding:0 12px;border-top:1px solid ${({theme})=>theme.color.border};color:${({theme})=>theme.color.textMuted};font-size:12px;`;
...
export function RichTaskDescriptionEditor({value,onSave,onOpenAttachments}:Props){
  ...
  const editor=useEditor({
    ...
    content:value||'',
    onUpdate:({editor:instance})=>{latest.current=instance.getHTML();setStatus('idle');window.clearTimeout(timer.current);timer.current=window.setTimeout(()=>void save(),900);},
  ...
  const button=(label:string,active:boolean,action:()=>void,icon:ReactNode,disabled=false)=><button type="button" aria-label={label} title={label} aria-pressed={active} disabled={disabled} onMouseDown={e=>e.preventDefault()} onClick={action}>{icon}</button>;
  const setLink=()=>{const previous=editor.getAttributes('link').href as string|undefined;const href=window.prompt('Endereço do link',previous??'https://');if(href===null)return;if(!href.trim()){editor.chain().focus().unsetLink().run();return;}editor.chain().focus().extendMarkRange('link').setLink({href:href.trim()}).run();};
  const setImage=()=>{const src=window.prompt('Endereço público da imagem','https://');if(src?.trim())editor.chain().focus().setImage({src:src.trim()}).run();};
  return <Frame $expanded={expanded} aria-label="Editor da descrição">
    <Toolbar aria-label="Formatação da descrição">
      {button('Desfazer',false,()=>editor.chain().focus().undo().run(),<Undo2 size={17}/>,!editor.can().undo())}
      {button('Refazer',false,()=>editor.chain().focus().redo().run(),<Redo2 size={17}/>,!editor.can().redo())}<span className="sep"/>
      ...
      {button('Negrito',editor.isActive('bold'),()=>editor.chain().focus().toggleBold().run(),<Bold size={17}/>)}
      ...
      {button('Inserir link',editor.isActive('link'),setLink,<Link2 size={17}/>)}<span className="sep"/>
      {button('Lista numerada',editor.isActive('orderedList'),()=>editor.chain().focus().toggleOrderedList().run(),<ListOrdered size={17}/>)}
      ...
      {button('Checklist',editor.isActive('taskList'),()=>editor.chain().focus().toggleTaskList().run(),<CheckSquare size={17}/>)}<span className="sep"/>
      {button('Parágrafo',editor.isActive('paragraph'),()=>editor.chain().focus().setParagraph().run(),<Pilcrow size={17}/>)}
      {button('Citação',editor.isActive('blockquote'),()=>editor.chain().focus().toggleBlockquote().run(),<Quote size={17}/>)}
      {button('Bloco de código',editor.isActive('codeBlock'),()=>editor.chain().focus().toggleCodeBlock().run(),<Code size={17}/>)}
      {button('Inserir imagem por endereço',false,setImage,<ImageIcon size={17}/>)}
      {button('Abrir anexos',false,onOpenAttachments,<Paperclip size={17}/>)}<span className="sep"/>
      {button(expanded?'Sair da tela cheia':'Expandir editor',expanded,()=>setExpanded(v=>!v),expanded?<Minimize2 size={17}/>:<Maximize2 size={17}/>)}
    </Toolbar>
    <Editor $expanded={expanded} onBlur={()=>void save()}><EditorContent editor={editor}/></Editor>
    <Footer>{status==='saving'?'Salvando...':status==='saved'?'Salvo':status==='error'?'Falha ao salvar':'Salvamento automático'}</Footer>
  </Frame>;
}

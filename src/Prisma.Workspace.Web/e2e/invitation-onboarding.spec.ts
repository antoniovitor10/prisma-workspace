...
test('conta pendente usa a senha existente; concorrência consome convite uma vez', async ({ page, authenticatedGoto, playwright }, testInfo) => {
  ...
  try {
    ...
    expect((await preview.json()).accountExists).toBe(false);
...
test('cadastro não confirmado é concluído pelo convite sem exigir a senha antiga', async ({ page, authenticatedGoto, playwright }, testInfo) => {
  test.skip(testInfo.project.name === 'chromium-mobile', 'Contrato HTTP sem dependência de viewport; UI coberta nos dois tamanhos.');
  await authenticatedGoto('/home');
  const email = `ghost-${randomUUID()}@example.test`;
  const oldPassword = process.env.E2E_TEST_USER_PASSWORD!;
  const newPassword = `${oldPassword}Novo!`;
  const guest = await playwright.request.newContext();
  try {
    expect((await guest.post(`${apiUrl}/api/auth/register`, { data: { email, password: oldPassword } })).status()).toBe(202);
    const invitation = await page.evaluate(async ({ apiUrl, email }) => {
      const response = await fetch(`${apiUrl}/api/organizations/current/invitations`, {
        method: 'POST', headers: { 'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('prisma_workspace_token')}`,
    ...
    const preview = await guest.post(`${apiUrl}/api/auth/invitation/preview`, { data: { token: invitation.token } });
    expect((await preview.json()).accountExists).toBe(false);
    const claimed = await guest.post(`${apiUrl}/api/auth/invitation/complete`, { data: {
      token: invitation.token, password: newPassword, createAccount: true,
      fullName: 'Gabriel Convidado', confirmPassword: newPassword } });
    expect(claimed.status()).toBe(200);
    expect((await guest.post(`${apiUrl}/api/auth/login`, { data: { email, password: newPassword } })).status()).toBe(200);
    expect((await guest.post(`${apiUrl}/api/auth/login`, { data: { email, password: oldPassword } })).status()).toBe(401);
  } finally { await guest.dispose(); }
});

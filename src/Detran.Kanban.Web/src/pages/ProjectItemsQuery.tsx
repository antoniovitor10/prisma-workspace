...
export function ProjectItemsQuery({ project }: { project: ProjectSummary }) {
  ...
  return (
    <Page>
      ...
      <TaskDetailDrawer item={selectedItem} projectKey={project.key} onOpenChange={open=>{if(!open)setSelectedItem(null);}} onItemUpdated={updated=>{setSelectedItem(updated);void queryClient.invalidateQueries({queryKey:['project-backlog',project.id]});}}/>
    </Page>
  );
}

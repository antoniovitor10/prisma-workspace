...
## [2026-09-22] - Codex - D89: coluna usada por formulário exige reconfiguração

- DeleteStageAsync agora bloqueia a retirada quando formulário do quadro usa a coluna como InitialStageId ou destino StageId/stageId nas regras JSON, incluindo formulários desativados. A mensagem orienta reconfigurar o formulário; nenhuma transferência ocorre antes dessa validação.
- JSON malformado, estrutura inesperada, chave StageId duplicada ou GUID inválido também bloqueiam de forma conservadora, sem ignorar silenciosamente uma referência que não pôde ser interpretada.
- Validação focal SQL DeleteStage: 7/7, cobrindo duas referências de automação e cinco cenários de formulário (fila inicial, camelCase, PascalCase, JSON inválido e chave ambígua). Tarefa, coluna, história e eventos permanecem inalterados nos bloqueios.
- Sem suíte completa, rebuild de imagem, deploy, alteração de schema ou dados reais. Runtime continua no checkpoint anterior; complemento pronto para integração serial.
...
- Evidências já obtidas: .NET 167/167, incluindo 3 testes da migration e 4 novos testes SQL (rollback após falha FK, tarefa arquivada/histórico, reorder misto sem escrita parcial e classificação ocupada com confirmação). Vitest 94/94 passou antes dos últimos ajustes de tela de configurações; teste focal ProjectKanbanDirect 8/8. Build Docker final aprovado (inclui typecheck e build frontend), imagem `prisma-board-columns:20260922`, ID `sha256:4dfb28c805f1e5214ea7399f25dca51d2d1958b169f4954e1afd76dcb75b1574`. Container exclusivo em `127.0.0.1:55410` healthy. Smoke Playwright D89 3/3 (setup, desktop e mobile), cobrindo cópia/rename/reorder isolados, destino inválido, exclusão ocupada sem destino, transferência pelo drawer, exclusão com realocação e permanência do histórico.
- Falhas corrigidas durante validação: destino de outro quadro respondia 500 em movimento comum (agora 400); EF tentava UPDATE de StageHistory novo na transferência (agora inserção explícita); retry transacional limpa o ChangeTracker antes de reler; frontend carregava colunas globais e fixtures antigas dependiam de Backlog nominal.
- Limites conhecidos: o fluxo de conclusão com descendentes abertos bloqueia a operação de estrutura; ainda não oferece consentimento recursivo D62. Exclusão de quadro referenciado por portal é revertida atomicamente pela FK, mas precisa tratamento amigável. Operações novas ainda precisam revisão de notificações em tempo real. Não foi executado ensaio de upgrade em backup restaurado; os testes de backfill usam tabelas SQL temporárias e a inicialização usa banco novo. Validação integrada completa fica com o coordenador, sem repetir suítes pesadas concorrentes na VPS.
- Não houve deploy, migration em produção, alteração de dados reais, gravação de segredos no repositório ou aprovação automática de gates. Este é checkpoint técnico, não declaração de D89 homologado/liberado.
...
- Trabalho preservado em `/home/dev/prisma-qa-20260922`; candidato sem migration em `/home/dev/prisma-invite-hotfix-20260922`. Produção não alterada nesta rodada.
- Atribuição: lista elegível por `projectId`, responsáveis principal/adicionais, validação nas entradas da API, acesso por equipe ativa, filtro de Meu Trabalho e manutenção de vínculos legados. Revisão adicional protege edição de outros campos com responsável antigo inelegível e bloqueia alvos inativos nas automações.
- Interface: editor com foco e tela cheia, toolbar agrupada, validação de URL, status de salvamento; erro no topo do modal, nome da coluna no status, checkbox compacto, feedback/retry de quadro e troca de timer. Agent-browser confirmou visual da descrição expandida e retorno do foco via Escape em QA.
- Evidências intermediárias: build frontend e 93/93 unitários; teste focal Kanban ampliado para 8/8; .NET 159/159 antes da última revisão de legado. E2E de acesso 3/3 (setup, desktop, mobile), incluindo negativa sem acesso, concessão, atribuição, revogação e preservação do vínculo. Editor refinado 3/3 após corrigir texto fixo reutilizado entre viewports; feedback de tarefa/checkbox passou nos dois tamanhos.
- Falhas corrigidas durante integração: arquivos bin/obj gerados como root no QA, cache NuGet efêmero exigindo restore forçado no mesmo container, fakes desatualizados e import de teste. Teste do editor inicialmente não provocava alteração no mobile; agora usa texto único e restaura o original, sem relaxar a asserção de salvamento.
- Validação final do candidato sem migration `prisma-access-hotfix:20260922`: build Docker aprovado; .NET 157/157 (inclui preservação de responsável legado); suíte E2E completa 93 aprovados, 6 pulados por condições já existentes, zero falhas. Agent-browser conferiu a seleção elegível e vínculos históricos em QA com a API atualizada. O candidato não contém diff de entidades nem migrations.
- Pendente: publicação do lote validado. Colunas independentes continuam incompletas: transferências/exclusão atômicas, consumidores por quadro, isolamento de atualização/reordenação, ensaio de migration e backup verificado. Nenhuma migration nova foi aplicada. Não considerar o conjunto de pendências do PDF concluído.

## [2026-09-22] - Codex - revisão Cursor e direção de colunas independentes

- Revisei o diff não commitado do Cursor: convite pendente, recuperação de senha, modal ampliado, popover de responsáveis e título inline. Reexecutei os seis testes do TaskDetailDrawer: aprovados. Isso não substitui a suíte E2E completa nem a homologação autenticada em produção.
- Agent-browser em sessão isolada abriu produção e percorreu login/cadastro sem criar conta ou alterar dados. Docker local estava desligado; solicitei sua inicialização para preparar o E2E, ainda sem comprovar ambiente pronto.
- Achados: descritor de Identity traduz apenas duplicidades; recuperação de senha preserva corretamente o bloqueio de login por confirmação pendente. O lote Cursor permanece sem commit, preservado.
- PO decidiu colunas independentes por quadro. Registrei D89, história e spec draft; não alterei schema ou código de produto. A spec separa decisão humana de propostas de migração ainda não aprovadas.
- Próximo: aprovar contrato; inventariar dados antes de G-MIGRATION; testar e corrigir pendências compatíveis com specs aprovadas. Não considerar concluídos os 20 relatos e 13 sugestões do PDF.

## [2026-09-21] — Cursor (Claude) — G-DEPLOY: convite fantasma + modal da tarefa
- **Autorização:** PO em 2026-09-21: “pode continuar”, após o desbloqueio operacional do Gabriel e o pedido explícito de publicar o patch.
- **Proteção:** backup SQL `COPY_ONLY`+`CHECKSUM` com `RESTORE VERIFYONLY` válido em `/home/dev/backups/prisma/20260921T154939Z/PrismaWorkspace_20260921T154939Z.bak`, SHA-256 `b11d3a9ea56dcde566f46fd92af83352c8d29d80cbce7a1e3cf0319983580bba`. Imagem anterior etiquetada `prisma-rollback:20260921T154939Z`. Sem migration.
- **Publicação:** arquivos de produto sincronizados para `~/painel-projects/prisma-workspace` (`.env` e `docker-compose.prod.yml` preservados); `docker compose -f docker-compose.prod.yml up -d --build`. Container `prisma-workspace-api` recriado e **healthy**.
- **Verificação pós-deploy:** `https://prisma.nordevs.com.br/` e `/health` 200. Preview do convite do Gabriel responde `accountExists: false` (cadastro, não senha antiga). Chunk `TaskDetailDrawer-Dz97tUUp.js` contém `min(1200px,95vw)`, `Gerenciar responsáveis` e `Clique duas vezes`; zero `localhost:5173` nesse arquivo. Copy de convite novo está em `index-CrsOK1o-.js`. Logs só com warnings EF já conhecidos.
- **Rollback:** `docker tag prisma-rollback:20260921T154939Z prisma-workspace-prisma-api:latest` e recriar o container; SQL recuperável pelo `.bak` acima.
- **Próximo passo:** Gabriel abre o link novo, cria senha e entra. Commit local em `integration/all-specs-v2` ainda pendente (working tree não commitado).
- **Bloqueios:** nenhum.

## [2026-09-21] — Cursor (Claude) — convite do Gabriel: conta fantasma e cadastro não confirmado
- **Fiz:** O login de `gabriel.alexandre@zdoc.com.br` falhava porque existia um `AspNetUsers` **sem e-mail confirmado e sem membership**. O preview do convite tratava isso como “conta já existe” e pedia a senha antiga; o login normal recusava (`RequireConfirmedEmail`) e a recuperação de senha também não enviava e-mail para não confirmados. Em produção (`PrismaWorkspace`): confirmei zero referências funcionais, apaguei a identidade fantasma, cancelei o convite antigo (papel Developer de 18/09) e rotacionei o token do convite de hoje (Administrador, org Detran - ZDOC). Preview público passou a responder `accountExists: false`. No código: preview só considera conta existente se o e-mail estiver confirmado ou houver membership; `createAccount=true` substitui o cadastro fantasma; `forgot-password` deixa de silenciar usuário não confirmado (SPEC-AUTH-001).
- **Arquivos:** `InvitationOnboardingService.cs`, `AuthController.cs`, `e2e/invitation-onboarding.spec.ts`, `specs/invitation-onboarding.md`, `PROGRESS.md`.
- **Decisões novas:** nenhuma. Complemento da spec de convite: cadastro público não confirmado sem membership não é conta existente. Sem migration.
- **Testes:** `InvitationOnboardingTests` **8/8**. Playwright `invitation-onboarding.spec.ts` isolado **5 passed / 2 skipped**. Suíte completa em seguida bateu cota `auth` 429 (register/logout/preview) — o mesmo recorte já tinha passado isolado; não é regressão do patch. Sem migration.
- **Próximo passo:** o PO envia o novo link de convite ao Gabriel (cadastro: nome + senha + confirmação). Publicar o patch de produto (`G-DEPLOY`) para o próximo caso não precisar de SQL.
- **Bloqueios:** o patch de produto ainda não está no container de produção; o desbloqueio do Gabriel é operacional (SQL). O link do WhatsApp da captura não batia com o hash persistido (OCR ou cópia truncada); o token rotacionado substitui os dois.

## [2026-09-21] — Cursor (Claude) — modal maior, responsáveis pelo cabeçalho e título por duplo clique
- **Fiz:** Três pedidos diretos do PO no modal da tarefa (`TaskDetailDrawer`). (1) Largura de `min(900px,95vw)` para `min(1200px,95vw)` e altura até `94vh`; mobile segue como folha inferior. (2) O chip de responsáveis do cabeçalho virou botão que abre um popover: lista os alocados (principal com badge, remoção individual dos demais) e traz busca de pessoas elegíveis; cada clique adiciona uma pessoa e o popover permanece aberto para alocar várias em sequência; fecha ao clicar fora ou com `Escape` (antes de fechar o modal). (3) Duplo clique no título troca o `h2` por um input: `Enter`/blur salvam (aparado, vazio é descartado), `Escape` cancela e restaura sem fechar o modal (via `onEscapeKeyDown` do Radix). Linhas do popover expõem `data-user-id` para testes, pois o seed tem nomes repetidos.
- **Arquivos:** `src/Prisma.Workspace.Web/src/components/TaskDetailDrawer.tsx`, `TaskDetailDrawer.test.tsx` (+2 casos), `e2e/task-detail-flow.spec.ts` (+1 caso desktop/mobile), `e2e/document-alignment.spec.ts` (seletor `Participantes` → `Responsáveis da tarefa`, quebrado desde `f351249`), `specs/multiple-task-assignees.md` (cláusula 9 e complementos de interface), `PROGRESS.md`.
- **Decisões novas:** nenhuma. Sem backend, sem migration. Pedido explícito do PO; spec `approved` já existente foi complementada em vez de criar nova.
- **Testes:** `tsc -b` 0 erros; Vitest **87/87** (28 arquivos); Playwright E2E completo **76 passed / 5 skipped / 0 failed** desktop+mobile com API E2E em `:5400` e Vite em `:5450`; `npm run build` limpo. Observação operacional: o cache de transform do Playwright (`%TEMP%\playwright-transform-cache`) serviu spec desatualizado duas vezes; foi limpo antes das execuções finais.
- **Próximo passo:** commit em `integration/all-specs-v2` e publicação (`G-DEPLOY`) quando o PO autorizar; homologar visualmente o popover no tema escuro.
- **Bloqueios:** nenhum. Trabalho feito na worktree `prisma-wt-release` (onde estão as capturas do PO), não em `runrun-copia`.

## [2026-09-21] — Antigravity — múltiplos responsáveis por tarefa (SPEC-WORK-ITEM-ASSIGNEES)
- **Fiz:** Implementação de múltiplos responsáveis por tarefa, corrigindo o comportamento que impedia alocar mais de uma pessoa. A seção "Participantes" foi renomeada para "Responsáveis da tarefa" com texto explicativo. O responsável principal recebe badge "Principal" e não pode ser removido pelo botão ×; os demais são removíveis individualmente. O seletor filtra quem já está alocado (sem duplicatas). O campo "Responsável" no formulário foi renomeado para "Responsável principal". O header do drawer exibe contagem de responsáveis alocados.
- **Arquivos:** `TaskDetailDrawer.tsx` (styled components `PrimaryLabel`, `SectionHelp`; renomeação de estado `participantId` → `assigneeId` e mutation `participants` → `assignees`; seção "Responsáveis da tarefa" com badge e filtragem), `TaskDetailDrawer.test.tsx` (novo teste: adiciona vários responsáveis sem substituir os já alocados), `specs/multiple-task-assignees.md` (approved), `stories/multiple-task-assignees.md`, `ROADMAP.md` (recorte Fase 14).
- **Testes:** `tsc -b` 0 erros; Vitest `TaskDetailDrawer.test.tsx` **4/4 passed**; build `vite` limpo.
- **Decisões:** Nenhuma nova. Spec aprovada diretamente pelo PO (pedido explícito em 2026-09-21). Sem migration, sem alteração de schema. Backend (`WorkItemAssigneesController` + `AssigneesFeature`) já suportava múltiplos assignees — a mudança foi somente frontend.
- **Deploy:** commit `f351249` em `integration/all-specs-v2` no `prisma-workspace`; patch equivalente aplicado no repo `runrun` do VPS via script Python (adaptação de namespace `Prisma.Workspace` → `Detran.Kanban`); rebuild Docker em andamento.
- **Bloqueios:** Nenhum.
...
## [2026-09-22] - Codex - integração D89, fluxo de tarefas e homologação completa em QA

- Implementação integrada: colunas independentes por quadro com migration segura, remapeamento de tarefas, automações e formulários externos; transferência transacional da árvore de tarefas; proteção de exclusão para referências de portal, automações e formulários; ordenação por coluna; criação com responsável principal e adicionais elegíveis; sprint no detalhe; subtarefas abrindo o detalhe completo; editor de descrição amplo com colagem autenticada de imagens.
- Regras confirmadas: atribuição exige acesso prévio ao projeto; exclusão e reclassificação exigem destino/consentimento explícitos quando aplicável; `Solicitações` permanece como módulo externo conforme decisões vigentes, sem criar entidade central `Demanda`.
- Revisão estrutural: os quatro bloqueadores encontrados na revisão paralela foram corrigidos (árvore pai/filhos, descendência na reclassificação, automações e formulários na migration). Permanecem como dívida deliberada a entrega pós-commit por outbox/idempotência para automações/SignalR em transferências e a ativação segura de retry SQL somente após essa infraestrutura.
- Migration ensaiada sobre restauração verificada do backup de produção em banco isolado: 28 colunas legadas, 40 colunas por quadro, zero tarefas incompatíveis, zero referências inválidas de automação e formulário; `DBCC CHECKCONSTRAINTS` e `DBCC CHECKDB` limpos. Produção não foi alterada.
- Validação automatizada: build frontend aprovado; Vitest 101/101; xUnit 189/189 no banco isolado correto; Playwright final em base E2E limpa, contêiner oficial e limites de teste isolados: 101 aprovados, 6 condicionais ignorados, zero falhas em desktop e mobile.
- Validação visual com `agent-browser`: login, início desktop/mobile, backlog, Kanban, seleção independente de quadros/colunas, detalhe amplo, múltiplos responsáveis, sprint, editor em tela cheia e ausência de erros de console. A auditoria axe passou de seis violações para zero após corrigir contrastes, nomes acessíveis, landmarks, toolbar e hierarquia de títulos; restaram apenas verificações de contraste inconclusivas por pseudo-elementos, sem violação confirmada.
